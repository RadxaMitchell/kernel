	...
	while (dataCount != (count + keyAddCount) && times-- > 0)
	{
		...
		if (raw[0] == 6)
		{
			...
			if (isFillData[index] == 0)
			{
				...
				dataCount++;
#ifdef ST_RAWTEST_LOGFILE
				snprintf(data_buf, ST_LOGFILE_MAX_LENGTH, "index%d :",index);
...
#ifdef ST_RAWTEST_LOGFILE
					snprintf(data_buf, ST_LOGFILE_MAX_LENGTH, "%d,",rawI);
					vfs_write(filp, data_buf, strlen(data_buf), &pos);
#endif

					STX_INFO("Sensor RAW %d,%d = %d", index, i, rawI);
					if (rawI > MAX_RAW_LIMIT || rawI < MIN_RAW_LIMIT)
					{
#ifdef ST_RAWTEST_LOGFILE
						snprintf(data_buf, ST_LOGFILE_MAX_LENGTH, "Error: Sensor RAW %d,%d = %d out of limity (%d,%d)\n", index, i, rawI, MIN_RAW_LIMIT, MAX_RAW_LIMIT);
						vfs_write(filp, data_buf, strlen(data_buf), &pos);
#endif
		...
		else if (raw[0] == 7)
		{
			...
			if (isFillData[count] == 0)
			{
				...
				dataCount++;
#ifdef ST_RAWTEST_LOGFILE
				snprintf(data_buf, ST_LOGFILE_MAX_LENGTH, "key :");
				vfs_write(filp, data_buf, strlen(data_buf), &pos);
#endif
				for (i = 0; i < tMode[3]; i++)
				{
					...
					{
#ifdef ST_RAWTEST_LOGFILE
						snprintf(data_buf, ST_LOGFILE_MAX_LENGTH, "Error: Key RAW %d = %d out of limity (%d,%d)\n",i, rawI, MIN_RAW_LIMIT, MAX_RAW_LIMIT);
						vfs_write(filp, data_buf, strlen(data_buf), &pos);
#endif
						STX_ERROR("Error: Key RAW %d = %d out of limity ", i, rawI);
