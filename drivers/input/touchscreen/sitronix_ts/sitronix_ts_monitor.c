...
#define SITRONIX_MT_DIS_LIMIT 2000
...
static int sitronix_ts_monitor_thread(void *data)
{
	...
	STX_INFO("%s start and delay %d ms", __func__, sitronix_ts_delay_monitor_thread_start);
	...
	while (!kthread_should_stop())
	{
		...
		else
		{
			...
			ret = stx_i2c_read_bytes(0x40, disbuf, 4 + (stx_gpts.ts_dev_info.tx_chs * 2));
			...
			STX_DEBUG("monitor sensing counter: %02x %02x", buf[6], buf[7]);
			...
			{
				result = 1;
				if (PreCheckData[0] == buf[6] && PreCheckData[1] == buf[7])
					StatusCheckCount++;
				else
					StatusCheckCount = 0;
			...
			if (disbuf[0] == 0x86)
			{
				...
				for (i = 0; i < stx_gpts.ts_dev_info.tx_chs; i++)
				{
					...
					{
						STX_ERROR("MT get error Distance for (%d,%d) , distance value = %d", disbuf[2], i, disv);
						disOK = false;
						break;
					}
				}
				...
				if (3 <= DisCheckCount)
				{
					STX_ERROR("Distance error for 3 times! ");
					result = -1;
					DisCheckCount = 0;
				}
			}
		...
		exit_i2c_invalid:
			if (0 == result)
			{
				...
				if ((2 <= i2cErrorCount))
				{
					STX_ERROR("I2C abnormal or status bootcode, reset it!");
					st_reset_ic();
#ifdef ST_GLOVE_SWITCH_MODE
					if (stx_gpts.glove_mode)
						st_enter_glove_mode(&stx_gpts);
