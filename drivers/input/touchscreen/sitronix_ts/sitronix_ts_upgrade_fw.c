...
{
	if (data[0x00] != 'S' || data[0x01] != 'T' || data[0x02] != 'F' || data[0x03] != 'W')
	{
...
int st_check_fw(const char *data, int *fwOff, int *fwSize, int *fwInfoOff, int *cfgOff)
{
	int off = 0;
	int devParamTabSize;
	int progSizeH;
	...
	*cfgOff = CT_CFG_OFF;
	...
	{
		*fwInfoOff = off + 4;
		*fwSize = 4 + data[off + 4] + off + 1 + 4; //tag(4) + TableSize + off + TableSizeOff + CksLen
...
int st_flash_unlock(void)
{
	...
	while (isSuccess == 0 && retryCount++ < ST_ISP_RETRY_MAX)
	{
		...
		if (st_i2c_upg_write_bytes(PacketData, ISP_PACKET_SIZE) == ISP_PACKET_SIZE)
		{
			...
			if (st_i2c_upg_read_direct(PacketData, ISP_PACKET_SIZE) == ISP_PACKET_SIZE)
			{
...
int st_flash_read_page(unsigned char *Buf, unsigned short PageNumber)
{
	...
	{
		if ((ReadLength = st_i2c_upg_read_direct(Buf + ReadNumByte, ISP_PACKET_SIZE)) != ISP_PACKET_SIZE)
		{
...
int st_flash_write_page(unsigned char *Buf, unsigned short PageNumber)
{
	...
	while (RetryCount++ < 1)
	{
		...
		if ((PacketData[4] != (Checksum >> 8)) || (PacketData[5] != (Checksum & 0xFF)))
		{
			STX_ERROR("Checksum error , ISP output = 0x%02X%02X , driver = 0x%04X", PacketData[4], PacketData[5], Checksum);
			return -1;
		}
...
static int sitronix_do_upgrade(void)
{
	...
	if (powerfulWrite == 0)
	{
		...
		{
			st_flash_read_page(&fw_check[ST_FLASH_PAGE_SIZE],((fwInfoOff / ST_FLASH_PAGE_SIZE) + 1));
			STX_INFO("read fwInfo");
		}

		if (0 == st_compare_array(fw_check + (fwInfoOff - checkOff), dump_buf + fwInfoOff, fwInfoLen))
		{
		...
		if (0 == st_compare_array(fw_check + (cfgOff - checkOff), dump_buf + cfgOff, cfgSize))
		{
...
int st_upgrade_fw(void)
{
	...
	checkOff = ISP_ID_OFFSET - (ISP_ID_OFFSET/ST_FLASH_PAGE_SIZE)*ST_FLASH_PAGE_SIZE;
	STX_INFO("ISP ID checkoff 0x%x , 0x%x \n",fw_check[checkOff],fw_check[checkOff + 1]);
