...
#if(defined(SITRONIX_GESTURE) ||defined(ST_SMART_WAKE_UP))
exit_data_handled:
#endif
	stx_irq_enable();
}
...
static irqreturn_t sitronix_ts_irq_handler(int irq, void *dev_id)
{
	...
	if((stx_gpts.irq_is_enable == 0)||(stx_gpts.irq_registered == 0))
	{
		spin_unlock_irqrestore(&stx_gpts.irq_lock, flags);
							 ...
							 struct sitronix_ts_platform_data *pdata)
{
	...
	STX_INFO("%s,pdata->irq_gpio=%d", __FUNCTION__, pdata->irq_gpio);
